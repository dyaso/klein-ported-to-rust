...
#[inline]
pub fn ext00(a: __m128, b: __m128, p1_out: &mut __m128, p2_out: &mut __m128) {
    // (a1 b2 - a2 b1) e12 +
    // (a2 b3 - a3 b2) e23 +
    // (a3 b1 - a1 b3) e31 +
    ...
    unsafe {
        *p1_out = _mm_mul_ps(a, _mm_shuffle_ps(b, b, 120 /* 1, 3, 2, 0 */));
        let um = _mm_sub_ps(
            *p1_out,
            _mm_mul_ps(_mm_shuffle_ps(a, a, 120 /* 1, 3, 2, 0 */), b),
        );
        *p1_out = _mm_shuffle_ps(um, um, 120 /* 1, 3, 2, 0*/);
...
#[inline]
pub fn ext_pb(a: __m128, b: __m128) -> __m128 {
...
pub fn ext02(a: __m128, b: __m128) -> __m128 {
    ...
    unsafe {
        let mut p3_out = _mm_mul_ps(a, _mm_shuffle_ps(b, b, 120 /* 1, 3, 2, 0 */));
        let um = _mm_sub_ps(
            p3_out,
            _mm_mul_ps(_mm_shuffle_ps(a, a, 120 /* 1, 3, 2, 0 */), b),
        );
        p3_out = _mm_shuffle_ps(
            um, um,
            //_mm_sub_ps(p3_out, _mm_mul_ps(_mm_shuffle_ps(a,a, 120 /* 1, 3, 2, 0 */), b)),
            120, /* 1, 3, 2, 0 */
        );
