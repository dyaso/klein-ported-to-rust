...
let um = _mm_sub_ps(*p1Out, _mm_mul_ps(_mm_shuffle_ps(a, a, 120 /* 1, 3, 2, 0 */), b));
		*p1Out = _mm_shuffle_ps(
			 um,um,
			 120 /* 1, 3, 2, 0*/);

		*p2Out = _mm_mul_ps(_mm_shuffle_ps(a, a, 0 /* 0, 0, 0, 0 */), b);
		*p2Out = _mm_sub_ps(*p2Out, _mm_mul_ps(a, _mm_shuffle_ps(b, b, 0 /* 0, 0, 0, 0 */)));
	}
...
pub fn ext02(a: __m128, b: __m128) -> __m128
{
	...
	unsafe {
		...
		let um = _mm_sub_ps(p3Out, _mm_mul_ps(_mm_shuffle_ps(a,a, 120 /* 1, 3, 2, 0 */), b));
		p3Out = _mm_shuffle_ps(um,um,
