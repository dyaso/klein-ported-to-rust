...
use crate::detail::sse::{dp, hi_dp, hi_dp_ss, rcp_nr1, hi_dp_bc, rsqrt_nr1};
...
pub fn gp00(a:__m128 ,b: __m128 , p1_out :&mut __m128,  p2_out:&mut __m128){
	...
	unsafe{
		*p1_out = _mm_mul_ps(_mm_shuffle_ps(a,a, 121 /* 1, 3, 2, 1 */), _mm_shuffle_ps(b,b, 157 /* 2, 1, 3, 1 */));

		*p1_out = _mm_sub_ps(*p1_out,
								  _mm_xor_ps(_mm_set_ss(-0.),
												 _mm_mul_ps(_mm_shuffle_ps(a,a, 158 /* 2, 1, 3, 2 */),
																_mm_shuffle_ps(b,b, 122 /* 1, 3, 2, 2 */))));
		// Add a3 b3 to the lowest component
		*p1_out = _mm_add_ss(
			 *p1_out,
			 _mm_mul_ps(_mm_shuffle_ps(a,a, 3 /* 0, 0, 0, 3 */), _mm_shuffle_ps(b,b, 3 /* 0, 0, 0, 3 */)));
		...
		*p2_out = _mm_sub_ps(*p2_out, _mm_mul_ps(a, _mm_shuffle_ps(b,b, 0 /* 0, 0, 0, 0 */)));
	}
}
...
pub fn gp03_flip(a: __m128, b:__m128, p1: &mut __m128, p2: &mut __m128){
	...
	unsafe {
		*p1 = _mm_mul_ps(a, _mm_shuffle_ps(b,b, 0 /* 0, 0, 0, 0 */));
		...
		// (_, a3 b2, a1 b3, a2 b1)
		*p2 = _mm_mul_ps(_mm_shuffle_ps(a,a, 156 /* 2, 1, 3, 0 */), _mm_shuffle_ps(b,b, 120 /* 1, 3, 2, 0 */));
		*p2 = _mm_sub_ps(
			*p2,
			_mm_mul_ps(_mm_shuffle_ps(a,a, 120 /* 1, 3, 2, 0 */), _mm_shuffle_ps(b,b, 156 /* 2, 1, 3, 0 */)));
...
pub fn gp11(a: __m128 ,b:__m128,p1Out:&mut __m128)
{
	...
	unsafe{
		...
		*p1Out = _mm_sub_ps(
			 *p1Out,
			 _mm_mul_ps(_mm_shuffle_ps(a,a, 121 /* 1, 3, 2, 1 */), _mm_shuffle_ps(b,b, 157 /* 2, 1, 3, 1 */)));
		...
		let tmp1
			 = _mm_mul_ps(_mm_shuffle_ps(a,a, 230 /* 3, 2, 1, 2 */), _mm_shuffle_ps(b,b, 2 /*0, 0, 0, 2*/));

		let tmp2
			 = _mm_mul_ps(_mm_shuffle_ps(a,a, 159 /*2, 1, 3, 3*/), _mm_shuffle_ps(b,b, 123 /*1, 3, 2, 3*/));
...
#[inline] pub fn gp33(a:__m128 , b:__m128 ) -> __m128{
	// (-a0 b0) +
	// (-a0 b1 + a1 b0) e01 +
	// (-a0 b2 + a2 b0) e02 +
	// (-a0 b3 + a3 b0) e03
...
pub fn gpRT(flip:bool,a: __m128 , b:__m128 ) -> __m128{
	unsafe{
		...
		if flip {
			...
			p2 = _mm_mul_ps(_mm_shuffle_ps(a,a, 1 /* 0, 0, 0, 1 */), _mm_shuffle_ps(b,b, 229 /* 3, 2, 1, 1 */));
			p2 = _mm_add_ps(
				p2,
				_mm_mul_ps(_mm_shuffle_ps(a,a, 122 /* 1, 3, 2, 2 */), _mm_shuffle_ps(b,b, 158 /* 2, 1, 3, 2 */)));
			p2 = _mm_sub_ps(p2,
				_mm_xor_ps(_mm_set_ss(-0.),
					_mm_mul_ps(_mm_shuffle_ps(a,a, 159 /*2, 1, 3, 3*/),
		...
		else
		{
			...
			p2 = _mm_add_ps(
				p2,
				_mm_mul_ps(_mm_shuffle_ps(a,a, 158 /* 2, 1, 3, 2 */), _mm_shuffle_ps(b,b, 122 /* 1, 3, 2, 2 */)));
			p2 = _mm_sub_ps(p2,
				_mm_xor_ps(_mm_set_ss(-0.),
					_mm_mul_ps(_mm_shuffle_ps(a,a, 123 /*1, 3, 2, 3 */),
...
pub fn gpLL(a:__m128, d:__m128, b:__m128, c:__m128, p1: &mut __m128, p2: &mut __m128){
	...
	unsafe{
		...
		*p1 = _mm_mul_ps(_mm_shuffle_ps(a,a, 217 /* 3, 1, 2, 1 */), _mm_shuffle_ps(b,b, 181 /* 2, 3, 1, 1 */));
		*p1 = _mm_xor_ps(*p1, flip);
		*p1 = _mm_sub_ps(*p1,
			 _mm_mul_ps(_mm_shuffle_ps(a,a, 183 /* 2, 3, 1, 3 */), _mm_shuffle_ps(b,b, 219 /* 3, 1, 2, 3 */)));
		let a2 = _mm_unpackhi_ps(a, a);
		let b2 = _mm_unpackhi_ps(b, b);
		*p1 = _mm_sub_ss(*p1, _mm_mul_ss(a2, b2));

		*p2 = _mm_mul_ps(_mm_shuffle_ps(a,a, 157 /* 2, 1, 3, 1 */), _mm_shuffle_ps(c,c, 121 /* 1, 3, 2, 1 */));
		*p2 = _mm_sub_ps(*p2,
							 _mm_xor_ps(flip,
											_mm_mul_ps(_mm_shuffle_ps(a,a, 123 /*1, 3, 2, 3 */),
		...
		*p2 = _mm_add_ps(
			 *p2,
			 _mm_mul_ps(_mm_shuffle_ps(b,b, 121 /* 1, 3, 2, 1 */), _mm_shuffle_ps(d,d, 157 /* 2, 1, 3, 1 */)));
		*p2 = _mm_sub_ps(*p2,
							 _mm_xor_ps(flip,
											_mm_mul_ps(_mm_shuffle_ps(b,b, 159 /*2, 1, 3, 3*/),
...
pub fn gpMM(a:__m128, b:__m128, c:__m128, d:__m128, e:&mut __m128, f:&mut __m128)
{
	unsafe{
		...
		*e = _mm_mul_ps(a_xxxx, c);
		...
		t = _mm_add_ps(t, _mm_mul_ps(a_zyzw, _mm_shuffle_ps(c,c, 2 /*0, 0, 0, 2*/)));
		...
		*f = _mm_add_ps(*f, _mm_mul_ps(a_ywyz, _mm_shuffle_ps(d,d, 121 /* 1, 3, 2, 1 */)));
		*f = _mm_add_ps(*f, _mm_mul_ps(_mm_shuffle_ps(b,b, 157 /* 2, 1, 3, 1 */), c_yzwy));
		t = _mm_mul_ps(a_zyzw, _mm_shuffle_ps(d,d, 2 /*0, 0, 0, 2*/));
		t = _mm_add_ps(t, _mm_mul_ps(a_wzwy, _mm_shuffle_ps(d,d, 159 /*2, 1, 3, 3*/)));
		t = _mm_add_ps(t, _mm_mul_ps(_mm_shuffle_ps(b,b, 2 /*0, 0, 0, 2*/), _mm_shuffle_ps(c,c, 230 /* 3, 2, 1, 2 */)));
		t = _mm_add_ps(t, _mm_mul_ps(_mm_shuffle_ps(b,b, 123 /*1, 3, 2, 3*/), c_wwyz));
		t = _mm_xor_ps(t, s_flip);
		*f = _mm_sub_ps(*f, t);
	}
}
