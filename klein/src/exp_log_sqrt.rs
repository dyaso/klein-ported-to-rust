...
use crate::detail::sse::{dp_bc, hi_dp, sqrt_nr1, rsqrt_nr1, rcp_nr1}; // hi_dp, hi_dp_bc, };
...
use crate::{Plane, Point, Line, IdealLine, Branch, Dual, Rotor, Translator, Motor};
...
impl Motor {
	...
	pub fn sqrt(self) -> Motor {
		unsafe {
			let mut m = Motor::from_rotor_and_translator(_mm_add_ss(self.p1_, _mm_set_ss(1.)), self.p2_);
    		m.normalize();
		    return m
		}
	}
...
mod tests {
    ...
    use crate::{Line, IdealLine, Plane, Point, Rotor, EulerAngles, ApplyOp, Motor, Branch, Translator};
